...
def ucob_progress(duration: datetime.timedelta):
...
class Fight:
    def __init__(self, start: datetime.datetime, end: datetime.datetime, location: str, status: str):
        self.start = start
        self.end = end
        ...
        for i, line in enumerate(self.lines):
            ...
            # detect end of fight
                ...
                if acl.actor_type in [ActorControlTypes.FadeIn, ActorControlTypes.Victory]:  # wipe or victory
                    ...
                    fight = Fight(fight_start, fight_end, fight_location, status)
                    in_fight = False
                    fights.append(fight)
...
def visualize(encounters: typing.List[Fight]):
    ...
    if all([e.location == 'TheUnendingCoilOfBahamutUltimate' for e in encounters]):
        data['progress'] = [ucob_progress(e.duration) for e in encounters]
